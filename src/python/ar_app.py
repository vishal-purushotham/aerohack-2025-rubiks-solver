...
class AeroHackARSolver:
    ...
    def run_ar_interface(self) -> Optional[str]:
        ...
        print("4. Use manual correction mode (M key) for precise color editing")
        print("5. Press SPACE to capture each face when detected properly")
        print("6. Follow the on-screen face scanning sequence")
        print("7. Professional solver will compute optimal solution!")
        print()
        print("Controls:")
        print("  SPACE - Capture current face")
        print("  C - Toggle calibration mode")
        print("  M - Toggle manual correction mode")
        print("  ESC - Exit scanner")
        print("  R - Reset scanning session")
        print("  In calibration: Ctrl+W/R/G/Y/O/B to force colors")
        print("  In manual mode: Click on preview tiles to cycle colors")
        print()
